...
- 本文完成後、`anti-ai-rewrite-master.md` の全項目を**適用**してリライト
- → **Skill: `output-check`** を実行（67点以上で合格、52点以下は再仕上げ）
- **output-check合格後に自動でStep 3へ進む**

**Step 3: 販売ページ設計**
→ **Skill: `paid-note-sales-page`**
- 元情報パッケージ + 記事の見出し一覧を入力として渡す
- タイトル最終案（5案+推奨）・サムネ文言・販売ページ全文を一括出力
- **Step 2のoutput-check合格後に自動実行**
- 販売ページ完成後、→ **Skill: `output-check`** を実行（67点以上で合格）
- ユーザーに確認・修正を依頼してから次へ進む
