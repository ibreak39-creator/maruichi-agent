...
**Step 6: AI臭さ除去チェック**
- `anti-ai-rewrite-master.md` の全項目を装飾後の本文に**適用**（装飾時にAI臭が再混入していないか確認）
- 装飾後の全体バランスを確認

**Step 7: 品質チェック（output-check）**
- → **Skill: `output-check`** を実行し、8軸80点満点で品質を評価
- **67点以上**: 合格 → workspaceに保存
- **53〜66点**: TOP3アクションを修正してから保存
- **52点以下**: Step 6に戻って再チェック。そのまま保存禁止
- ※ 装飾によって元記事の品質が下がっていないことを確認する目的。output-checkを通さずに保存してはならない
