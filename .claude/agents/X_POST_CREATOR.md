...
**文字数指定が最優先**:
- 「短文で」「140文字で」「短く」→ **`x-short-post`** を使う
- 「長文で」「詳しく」「ノウハウを」→ **`x-long-post`** を使う
- 指定がない場合は投稿内容で判断（下表参照）

| 投稿内容 | 使用スキル | 特徴 | 優先度 |
|---------|-----------|------|--------|
| 短文投稿（140文字以内）| x-short-post | フック→本文→CTAを1投稿で完結。3案提示 | **「短文で」と言われたら必ずこれ** |
...
**デフォルトスキル**: 文字数・投稿タイプの判断に迷った場合は **x-long-post** を使用する。
...
#### パターン0: 短文投稿（最優先分岐）
> トリガー: 「短文で」「140文字で」「短く」「短文投稿」
> ※ 文字数指定があれば他の条件より優先してこのパターンを使う

**Step 2: 投稿作成**
- → **Skill: `x-short-post`** で140文字以内の3案を作成
